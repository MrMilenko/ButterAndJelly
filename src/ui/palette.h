...
// palette.h: the colors, kept apart from render.h.
//
// Separate because render.h pulls in SDL_ttf, and things that only want to
// know what color something is should not have to have a font library. The
...
// The console's own color, used for its name in the title.
#if defined(__WIIU__)
...
constexpr Color Platform   { 0xF5, 0xA6, 0x23 };   // the rainbow's orange
#else
constexpr Color Platform   { 0x93, 0x98, 0xA8 };
#endif
constexpr Color Danger     { 0xE5, 0x5B, 0x5B };
constexpr Color Shadow     { 0x00, 0x00, 0x00, 0x99 };
// Behind subtitle text, where a soft shadow would not separate it from a
// bright picture.
constexpr Color Outline    { 0x00, 0x00, 0x00 };
}  // namespace Palette
