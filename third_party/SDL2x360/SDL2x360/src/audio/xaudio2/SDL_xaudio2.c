...
static int
XAUDIO2_OpenDevice(_THIS, const char *devname)
{
    HRESULT result = S_OK;
    ...
    */
    ...
    while ((!valid_format) && (test_format)) {
        ...
        case AUDIO_F32:
#ifdef __XBOX_360__
        /* Xenon is big endian and its XAudio2 takes PCM in native byte order.
           The formats above are all little endian, so nothing matched what an
           app asks for with AUDIO_S16SYS here, SDL fell back to AUDIO_S16LSB
           and byte swapped every sample on the way to the device -- which comes
           out of the speakers as static. */
        case AUDIO_S16MSB:
        case AUDIO_S32MSB:
        case AUDIO_F32MSB:
#endif
...
XAUDIO2_Init(SDL_AudioDriverImpl * impl)
{
    ...
    /*TODO: impl->PrepareToClose = XAUDIO2_PrepareToClose; */
...
AudioBootStrap XAUDIO2_bootstrap = {
    "xaudio2", "XAudio2", XAUDIO2_Init, SDL_FALSE
};
