...
/* The blobs above are PC ps_2_0 microcode and Xenos will not take them, so
 * CreatePixelShader fails, SDL quietly disables its YUV path, and video has to
 * be converted to RGB on the CPU -- which on this console is the difference
 * between a frame that fits in its budget and one that costs 80ms.
 *
 * D3DX is on the console and can compile HLSL at runtime, which is what
 * XBMC-360's video renderer does. No build-time shader compiler, no Wine, and
 * the source is legible instead of a wall of hex. ps_2_0 for the same reason:
 * it is the profile XBMC-360 compiles with on this hardware, and this shader
 * is three texture reads and a matrix, nowhere near its limits.
 *
 * Sampler registers match what SDL's own shaders use and what the renderer
 * binds: s0 luma, s1 and s2 the two chroma planes. The vertex colour arrives
 * in COLOR0 and is multiplied in, which is what carries SDL's colour and alpha
 * modulation.
 */
#include <d3dx9.h>

/* The XDK declares ID3DXBuffer but ships none of the ID3DXBuffer_* wrappers
 * the PC SDK has, so calling one links against a function that does not
 * exist. Go through the vtable, which is what those wrappers expand to.
 */
#define ID3DXBuffer_GetBufferPointer(p) ((p)->lpVtbl->GetBufferPointer(p))
#define ID3DXBuffer_Release(p)          ((p)->lpVtbl->Release(p))

#define D3D9_YUV_SHADER_BODY(YOFFSET, YSCALE, RV, GU, GV, BU) \
    ...
    "    float y = (tex2D(YTexture, In.Uv).r - " YOFFSET ") * " YSCALE ";" \
    "    float u =  tex2D(UTexture, In.Uv).r - 0.5;"          \
    "    float v =  tex2D(VTexture, In.Uv).r - 0.5;"          \
    "    float4 rgba;"                                        \
    "    rgba.r = y + " RV " * v;"                            \
    "    rgba.g = y + " GU " * u + " GV " * v;"               \
    "    rgba.b = y + " BU " * u;"                            \
    "    rgba.a = 1.0;"                                       \
    "    return rgba * In.Colour;"                            \
    "}"

/* Macros rather than variables: the array below is a static initialiser, and
 * a "const char *" is not a compile-time constant in C. */

/* Full range: JPEG video has no head or foot room, so luma is used as it is. */
#define D3D9_SOURCE_YUV_JPEG \
    D3D9_YUV_SHADER_BODY("0.0", "1.0", "1.402", "-0.344136", "-0.714136", "1.772")

/* Studio range, 16-235, with the two matrices everything else uses. */
#define D3D9_SOURCE_YUV_BT601 \
    D3D9_YUV_SHADER_BODY("0.0625", "1.164384", "1.596027", "-0.391762", "-0.812968", "2.017232")

#define D3D9_SOURCE_YUV_BT709 \
    D3D9_YUV_SHADER_BODY("0.0625", "1.164384", "1.792741", "-0.213249", "-0.532909", "2.112402")

static const char *const D3D9_sources[] = {
    D3D9_SOURCE_YUV_JPEG,
    D3D9_SOURCE_YUV_BT601,
    D3D9_SOURCE_YUV_BT709,
};

HRESULT D3D9_CreatePixelShader(IDirect3DDevice9 *d3dDevice, D3D9_Shader shader, IDirect3DPixelShader9 **pixelShader)
{
    const char *source = D3D9_sources[shader];
    ID3DXBuffer *code = NULL;
    ID3DXBuffer *errors = NULL;
    HRESULT result;

    result = D3DXCompileShader(source, (UINT)SDL_strlen(source), NULL, NULL,
                               "main", "ps_2_0", 0, &code, &errors, NULL);
    if (FAILED(result) || !code) {
        char line[96];
        SDL_snprintf(line, sizeof(line),
                     "[d3d] D3DXCompileShader failed, hr 0x%08x%s\r\n",
                     (unsigned)result, errors ? ":" : " (no message)");
        OutputDebugStringA(line);
        if (errors) {
            OutputDebugStringA((const char *)ID3DXBuffer_GetBufferPointer(errors));
            OutputDebugStringA("\r\n");
    ...
    result = IDirect3DDevice9_CreatePixelShader(
        d3dDevice, (const DWORD *)ID3DXBuffer_GetBufferPointer(code), pixelShader);
    ID3DXBuffer_Release(code);
    return result;
}
