...
#ifdef __XBOX_360__
    /* The 360 XDK reports SubType 0 for a standard controller, which desktop
       XInput uses for "unknown" -- so the checks below would discard the only
       controller the console has. */
...
void SDL_XINPUT_JoystickUpdate(SDL_Joystick *joystick)
{
    ...
    /* There is no XInputGetBatteryInformation on this console, so unlike the
       Windows backend there is nothing to fill this in from. It still has to be
       initialised, because UpdateXInputJoystickState reads it every frame:
       left as it is, it reports whatever the stack happened to hold, and any
       value other than BATTERY_TYPE_UNKNOWN sends the decoder off to fabricate
       a power level and push an SDL_JOYBATTERYUPDATED event every time the
       garbage changes. UNKNOWN specifically, not SDL_zero on its own -- zero is
       BATTERY_TYPE_DISCONNECTED, which the decoder treats as a real answer and
       reports as a flat battery. */
    SDL_zero(XBatteryInformation);
    XBatteryInformation.BatteryType = BATTERY_TYPE_UNKNOWN;

#if defined(__XBOXONE__) || defined(__XBOXSERIES__) || defined(__XBOX_360__)
    /* The 360 XDK leaves dwPacketNumber at 0 (as XInputOnGameInput does), so the
       change check below never fires and the joystick state stays frozen at
       whatever was read when the device was opened. Update every frame. */
